...
int main(int argc, char *argv[])
{
    ...
    cpu_option.cpu.cpu_num = 0;
    rv = PAPI_set_opt(PAPI_CPU_ATTACH, &cpu_option);
    if ( rv != PAPI_OK ) {
        std::cerr << "Failed to attach to CPU: " << rv << std::endl;
        exit(EXIT_FAILURE);
    }
...
//FIXME: Error: duplicated start returns value other than -10: 0
//    rv = SYSSAGE_PAPI_start(eventSet);
//    if ( rv != PAPI_EISRUN ) {
